...
class GrazHighResMNNBenchmark:
    ...
    def benchmark(self, wrapper, factor=1, save_stats=True):
        ...
        for scene_name in tqdm(self.scenes.keys(), desc="Processing scenes"):
            ...
            Ks = np.load(DATASET_PATH / scene_name / 'cameras/cameras.npz', allow_pickle=True)
            poses = np.load(DATASET_PATH / scene_name / 'cameras/poses.npz', allow_pickle=True)

            for pair_id, (img1_name, img2_name) in enumerate(tqdm(scene, desc=f"Processing scene {scene_name}")):
                try:
                    # Data preparation
                    ...
                    print(f"Error loading images {img1_name} and {img2_name} in scene {scene_name}: {e}")
                    continue
