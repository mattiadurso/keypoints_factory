...
for s in resolutions:
    for wrapper in wrappers_list:
        os.system(
            f"python {script_name} --benchmark-name ghr \
            --wrapper-name {wrapper} --max-kpts {max_kpts} \
            --run-tag {tag} --ransac-th {th} --min-score {min_score} --ratio-test {ratio_test} \
            --stats {stats} --ghr-partial {partial} --scaling-factor {scale_factors[s]}"
        )
